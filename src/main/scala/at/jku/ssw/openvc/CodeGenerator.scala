...
object CodeGenerator {
  ...
  private final case class Context(cw: RichClassWriter, mv: RichMethodVisitor, loopLabels: Map[(Int, Int), LoopLabels], designUnit: String)
  ...
  def apply(configuration: Configuration, sourceFileName: String, designFile: DesignFile): Unit = {
    ...
    def visitEntityDeclaration(entityDeclaration: EntityDeclaration): Unit = {
      ...
      acceptList(entityDeclaration.concurrentStatements, Context(cw = cw, mv = null, Map(), entityDeclaration.identifier.text))
      cw.writeToFile
    }
    ...
    def visitReturnStatement(returnStmt: ReturnStatement, context: Context): Unit = {
      ...
      returnStmt.expression match {
        case None => copyBackIfNeeded(returnStmt.procedureSymbol, mv)
        case Some(expr) =>
    ...
    def visitFunctionDefinition(functionDefinition: FunctionDefinition, context: Context): Unit = {
      ...
      startLabel()
      val newContext = Context(context.cw, mv, Map(), context.designUnit)
    ...
    def copyBackIfNeeded(procedureSymbol: ProcedureSymbol, mv: RichMethodVisitor): Unit = {
      import mv._
    ...
    def visitProcedureDefinition(procedureDefinition: ProcedureDefinition, context: Context): Unit = {
      ...
      copyBackIfNeeded(procedureSymbol, mv)
      stopLabel()
    ...
    def visitBlockStatement(blockStmt: BlockStatement, context: Context): Unit = {
      ...
      acceptList(blockStmt.statementList, Context(cw = cw, null, loopLabels = Map(), context.designUnit))
      cw.writeToFile
      error("not implemented")
    }
    ...
    def visitProcessStatement(processStmt: ProcessStatement, context: Context): Unit = {
...

      {
        ...
        continueLabel()
        acceptList(processStmt.sequentialStatementList, Context(cw = cw, mv = mv, loopLabels = Map(), context.designUnit))
