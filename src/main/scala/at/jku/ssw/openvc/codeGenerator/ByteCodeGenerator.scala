...
object ByteCodeGenerator {
  ...
  private final case class Context(cw: RichClassWriter, mv: RichMethodVisitor, loopLabels: Map[Position, LoopLabels], designUnit: String, fileSymbols: Seq[FileSymbol] = Seq())
  ...
  def apply(configuration: Configuration, sourceFileName: String, designFile: DesignFile) {
    ...
    def acceptExpression(expr: Expression, mv: RichMethodVisitor, contextOption: Option[ExpressionContext] = None, createDebugLineNumberInformation: Boolean = true) {
      ...
      def acceptExpressionInner(expression: Expression, innerContext: ExpressionContext = null): Unit =
        expression match {
          ...
          case FieldAccessExpression(symbol, field, fieldDataType, _, expression) =>
            ...
            mv.GETFIELD(symbol.dataType.implementationName, field.text, getJVMDataType(fieldDataType))
            acceptExpressionInner(expression, innerContext)
          case e =>
      ...
      def visitAttributeAccessExpression(attributeAccess: AttributeExpression) {
        attributeAccess.attribute match {
          case preDefinedAttributeSymbol: PreDefinedAttributeSymbol =>
            attributeAccess.symbol match {
                    ...
                    attributeAccess.attribute.name match {
                      ...
                      case "image" =>
                        ...
                        scalarType match {
                          ...
                          case enumType: EnumerationType => mv.INVOKESTATIC(enumType.implementationName, "image", "(I)Ljava/lang/String;")
                        }
                      case "value" =>
                        acceptExpressionOption(attributeAccess.expression, mv)
                        scalarType match {
                          ...
                          case enumType: EnumerationType => mv.INVOKESTATIC(enumType.implementationName, "value", "(Ljava/lang/String;)I")
                        }
              ...
              case signal: SignalSymbol if (signal.attributes.contains(attributeAccess.attribute.name)) =>
                ...
                mv.INVOKEVIRTUAL(getJVMName(signal), attributeAccess.attribute.name, if (preDefinedAttributeSymbol.parameter.isDefined) "(L)" else "()" + getJVMDataType(attributeAccess.attribute.dataType))
          ...
          case userDefinedAttributeSymbol: UserDefinedAttributeSymbol =>
            userDefinedAttributeSymbol.owner match {
              case _: PackageSymbol | _: EntitySymbol | _: ArchitectureSymbol | _: ConfigurationSymbol | _: ProcessSymbol =>
      ...
      def visitAggregate(aggregate: Aggregate) {
        //TODO
        val elements = aggregate.elements
        aggregate.dataType match {
          case arrayType: ConstrainedArrayType =>
            arrayType.dimensions.size match {
              case 1 =>
                ...
                arrayType.elementType match {
                  ...
                  case recordType: RecordType => mv.ANEWARRAY(recordType.implementationName)
                }
              case size =>
            ...
            for ((element, i) <- aggregate.elements.zipWithIndex) {
              mv.DUP
              mv.pushInt(i)
          ...
          case recordType: RecordType =>
            ...
            DUP
            for (element <- aggregate.elements)
      ...
      def visitFunctionCallExpression(functionCallExpr: FunctionCallExpression) {
        ...
        functionSymbol.attributes.get("foreign") match {
          case Some(attributeSymbol) if (attributeSymbol.isInstanceOf[UserDefinedAttributeSymbol] && attributeSymbol.asInstanceOf[UserDefinedAttributeSymbol].attributeDeclaration == SymbolTable.foreignAttribute) =>
            loadParameters(functionCallExpr.parameterAssociationList, mv)
            attributeSymbol.asInstanceOf[UserDefinedAttributeSymbol].value match {
          ...
          case _ =>
            ...
            loadParameters(functionCallExpr.parameterAssociationList, mv)
            mv.visitMethodInsn(functionCallType, functionSymbol.owner.implementationName, functionSymbol.name, "(" + getJVMParameterList(functionSymbol.parameters) + ")" + getJVMDataType(functionSymbol.returnType))
        }
        acceptExpression(functionCallExpr.expression, mv)
      }
      ...
      def visitPhysicalLiteral(physicalLiteral: PhysicalLiteral) {
        ...
        physicalLiteral.literalType match {
          case INTEGER_LITERAL => pushLong(physicalLiteral.toLong * dataType.units(physicalLiteral.unitSymbol.identifier.text))
          case REAL_LITERAL => pushLong((physicalLiteral.toDouble * dataType.units(physicalLiteral.unitSymbol.identifier.text)).toLong)
        }
      }
        ...
        case Left(range) =>
          range.expressionsOrName match {
            ...
            case Left((fromExpression, direction, toExpression)) =>
              ...
              val newContext = context.copy(loopLabels = context.loopLabels + (forStmt.position -> new LoopLabels(continueLabel, breakLabel)))
    ...
    def visitLoopStatement(loopStmt: LoopStatement, context: Context) {
      ...
      continueLabel()
      val newContext = context.copy(loopLabels = context.loopLabels + (loopStmt.position -> new LoopLabels(continueLabel, breakLabel)))
    ...
    def visitEntityDeclaration(entityDeclaration: EntityDeclaration) {
      val cw = createClass(Opcodes.ACC_ABSTRACT, entityDeclaration.symbol.implementationName, "java/lang/Object", classOf[EntityAnnotation])
    ...
    def storeSymbol(mv: RichMethodVisitor, symbol: RuntimeSymbol, targetType: DataType) = symbol.owner match {
      case _: ArchitectureSymbol | _: TypeSymbol | _: ProcessSymbol => mv.PUTFIELD(symbol.owner.implementationName, symbol.name, getJVMDataType(symbol))
      case _: PackageSymbol => mv.PUTSTATIC(symbol.owner.implementationName, symbol.name, getJVMDataType(symbol))
      case _: SubprogramSymbol => mv.storeInstruction(symbol)
      case r: RuntimeSymbol =>
        symbol.dataType match {
          case recordType: RecordType =>
            mv.PUTFIELD(recordType.implementationName, symbol.name, getJVMDataType(targetType))
          case accessType: AccessType =>
      ...
      varAssignStmt match {
        case stmt: SimpleVariableAssignmentStatement =>
          stmt.target.nameOrAggregate match {
            case Left(name) =>
              val (target, targetType) = loadTarget(mv, stmt.target.expression)
              target.owner match {
    ...
    def visitWhileStatement(whileStmt: WhileStatement, context: Context) {
      ...
      continueLabel()
      val newContext = context.copy(loopLabels = context.loopLabels + (whileStmt.position -> new LoopLabels(conditionTestLabel, breakLabel)))
    ...
    def visitProcedureCallStatement(procedureCallStmt: ProcedureCallStatement, context: Context) {
      ...
      procedureSymbol.attributes.get("foreign") match {
        case Some(attributeSymbol) if (attributeSymbol.isInstanceOf[UserDefinedAttributeSymbol] && attributeSymbol.asInstanceOf[UserDefinedAttributeSymbol].attributeDeclaration == SymbolTable.foreignAttribute) =>
          ...
          attributeSymbol.asInstanceOf[UserDefinedAttributeSymbol].value match {
            case Left((className, methodName)) => mv.INVOKESTATIC(className, methodName, "(" + getJVMParameterList(procedureSymbol.parameters) + ")V")
            case Right((className, methodName, parameterTypes)) => mv.INVOKESTATIC(className, methodName, parameterTypes)
          }
        case _ =>
          ...
          procedureSymbol.owner match {
            ...
            //implicitly declared procedures and functions
              ...
              procedureName.foreach{
                loadParameters(procedureCallStmt.parameterAssociationList, mv)
                mv.visitMethodInsn(procedureCallType, procedureSymbol.owner.implementationName, _, "(" + getJVMParameterList(procedureSymbol.parameters) + ")V")
              }
            case _ =>
              loadParameters(procedureCallStmt.parameterAssociationList, mv)
              mv.visitMethodInsn(procedureCallType, procedureSymbol.owner.implementationName, procedureSymbol.name, "(" + getJVMParameterList(procedureSymbol.parameters) + ")V")
          }
    ...
    def visitProcessStatement(processStmt: ProcessStatement, context: Context) {
      ...
      initItems(context.designUnit, 0, "init", processStmt.declarativeItems, cw);
      {
        ...
        visitLoopStatement(LoopStatement(Position.Empty, None, processStmt.sequentialStatementList, None), Context(cw = cw, mv = mv, loopLabels = Map(), designUnit = context.designUnit)) //infinite loop
        mv.RETURN
        mv.endMethod
      }
      cw.writeToFile()
