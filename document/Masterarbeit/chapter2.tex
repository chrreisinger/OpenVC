...
VHDL wurde ursprünglich entwickelt aufgrund der Forderung des US-Verteidigungsministerium, das komplexe Verhalten von Application Specific Integrated Circuits (ASIC) die Zulieferer in ihren Geräten einbauten, zu dokumentieren. So gesehen kann mann daher sagen, VHDL wurde als Alternative zu großen, komplexen Handbüchern die implementierungsspezifische Details enthalten, entwickelt.
...
Danach wurde erkannt, dass sich mit den vorhanden Datentypen \textit{bit} und \textit{boolean} "'multi-valued logic"', in dem  ein Signal die Zustände (undefiniert, schwach, stark) annehmen kann, nicht modellieren lassen kann. Dieses Problem wurde mit dem IEEE 1164 Standard gelöst, der die zusätzlichen 9-wertige logische Datentypen \textit{std\_ulogic} und \textit{std\_ulogic\_vector} definiert. \cite{VHDL_1164}

Im Jahr 1993 wurde eine Aktualisierung des Standards veröffentlicht, der vorallem die Syntax konsistenter machte, den Zeichensatz auf ISO-8859-1 erweiterte und den \textit{xnor} Operator einführte. \cite{VHDL93}
...
In VHDL wird der Source Code in Bibliotheken organisiert, wobei jede Bibliothek jeweils compilierten
...
\begin{itemize*}
	\item \textit{work} ist die Standard-Bibliothek des Benutzers. Wenn nicht anders spezifiziert, dann ist \textit{work} die Bibliothek, mit der die Programme arbeiten.
...
\subsection{Package}
Deklarationen die in mehreren Designs benutzt werden, z.B.: Unterprogramme (Funktionen, Prozeduren) oder Typen, Konstanten, lassen sich in \textit{packages} sammeln und in Bibliotheken hinterlegen. 
...
In der Zeile 21 kann man das Beispiel für einen überladenen Operator sehen.
...
In \ref{reg4EntitySymbol} sieht man das Schaltungssymbol.
...
\subsubsection*{Beispiel}
\todo{Besseres Beispiel und Erlärung}
...
Zusätzlich werden einige Erweiterungen des neuen 2008er Standards, die sich leicht implementieren lassen, akzeptiert.

Unter Anderem wird folgendes nicht unterstützt:
\begin{itemize*}
	\item Konfigurationen mit \textit{context declarations}
...
\begin{itemize*}
	...
	\item der condition operator ??
	\item verbesserte Bit-String Literale, z.B. 6Ux'"f"' für binär '"001111"'
	\item block comments (C-style Kommentare /* */)
	\item context declarations
	\item neue unäre logische Operatoren
	\item matching relational operator
\end{itemize*}
Zu den neuen Arten von Zuweisen gibt es später Beispiele wie der Compiler sie behandelt.